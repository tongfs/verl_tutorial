...
| 时段 | 内容 | 时长 |
|------|------|------|
| 0:00–0:30 | 过一遍思维导图，完成核心概念自检 | 30 分钟 |
| 0:30–0:35 | 休息 | 5 分钟 |
| 0:35–1:25 | 实践：Iris 或 MNIST 分类 | 50 分钟 |
| 1:25–2:00 | 对照检查清单查漏补缺，整理笔记 | 35 分钟 |
...
1. **理解**：神经网络前向/反向、梯度下降、过拟合与正则化
2. **使用**：PyTorch 张量、autograd、nn.Module、优化器
3. **实现**：独立完成一个「定义模型 + 训练循环 + 验证」的分类任务
4. **调参**：理解 batch size、lr、Dropout、weight_decay 的作用
...
第一周打下的「神经网络 + PyTorch」基础，是理解 Transformer 和 RLHF 的前提。
