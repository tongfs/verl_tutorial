...
| 类型 | 资源 | 链接/获取方式 | 建议用法 |
|------|------|---------------|----------|
| 视频 | 李宏毅「Proximal Policy Optimization」 | B 站搜「李宏毅 PPO」 | PPO-Clip 直观、clip 公式 |
...
## 五、第二部分：PPO 在 LLM 中的角色（约 1 小时）

### 5.1 Actor-Critic 与 Advantage

- Actor：策略 $\pi_\theta(a\mid s)$；Critic：价值函数 $V_\phi(s)$
- Advantage $A_t = Q(s_t,a_t) - V(s_t)$：用 $A$ 替代 $G$ 降低方差
- 在 LLM 中：Actor = 语言模型；Critic 用于估计 advantage

### 5.2 GAE 与 InstructGPT 中的 PPO

- GAE：$\hat{A}_t^{\text{GAE}} = \sum_{l=0}^{\infty} (\gamma\lambda)^l \delta_{t+l}$，$\lambda \approx 0.95$
- RLHF 中：Actor = 要优化的 LM，Ref = SFT 模型，reward 来自 RM
